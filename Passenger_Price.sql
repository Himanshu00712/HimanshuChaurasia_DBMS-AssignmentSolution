...
SELECT ps.Passenger_name, ps.Boarding_City, ps.Destination_City, pc.Bus_Type, pc.Price
FROM Passenger ps, Price pc
WHERE ps.Distance = pc.Distance
AND ps.Bus_Type = pc.Bus_Type
GROUP BY ps.Passenger_name;
...
SELECT p.passenger_name, (p.distance/(SELECT sum(distance) FROM passenger))*100 percentage FROM passenger p;
...
when (p.price >500 and p.price<1000) then 'Average Cost'
else 'Cheap' end  cost_category 
from price p;
